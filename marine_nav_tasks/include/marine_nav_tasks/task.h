#ifndef MARINE_NAV_TASKS_TASK_H
#define MARINE_NAV_TASKS_TASK_H
...
{
public:
  using TaskInformation = marine_nav_interfaces::msg::TaskInformation;

  virtual ~Task() = default;

  /// Creates a new Task.
  static TaskPtr create(const TaskInformation& task_msg, TaskPtr parent);

  static TaskPtr create(const TaskInformation& task_msg, rclcpp::Clock::SharedPtr clock);
  ...
  bool update(const TaskInformation& task_msg, bool check_id = true);
  ...
  void update(const std::vector<TaskInformation>& task_msgs);

  /// Returns TaskInformation message associated with this task. 
  const TaskInformation& message() const;
